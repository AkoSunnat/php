...
if ($data[1]==$prod_name && $data[2]==$brand && $data[3]==$category && $data[4]==$size && $data[5]==$color && $data[6]==$price){
    ...
    $query = "INSERT INTO 1z9Eejw0qq.product(prod_name,brand,category_id,size,color,price,quantity,img_url) VALUES ('$prod_name','$brand','$category','$size','$color','$price','$quantity','$img_url')";
    $res = mysqli_query($conn,$query);
